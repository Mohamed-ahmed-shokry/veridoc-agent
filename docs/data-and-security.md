...
one) but still requires the exact `Origin` match. Requests that reach the review
origin boundary return `503 review_authentication_unavailable` unless
`VERIDOC_REVIEW_ORIGIN` is configured as an HTTPS origin, so the `Secure`
session cookie is never expected to travel over plain HTTP. Missing session
cookies are rejected with `401` before that configuration boundary.
