...
├── test_deployment_maintenance.py   automated backup, retention, and maintenance
...
`test_review_models.py`, `test_review_transitions.py`, and
`test_review_authorization.py` exercise the strict domain schemas (including
digest-verified snapshot round-tripping and per-event-type shape rules), the
deterministic status-transition table, and the complete role/assignee
authorization matrix as pure functions, with no storage or HTTP involved.
`test_review_config.py` and `test_review_auth.py` cover actor-file/origin
environment validation and constant-time credential/session/CSRF token
handling in isolation.

`test_review_sqlite_migrations.py` and `test_review_schema.py` mirror the
reference-data persistence tests independently for the dedicated review
store. `test_review_sqlite_repository.py` uses a temporary SQLite file to
prove case creation, listing/filtering, every mutation's authorization and
`expected_version` guard, and idempotent replay versus conflict.
`test_review_persistence_concurrency.py` races threads against one case to
prove exactly one writer wins a version or idempotency-key race and no
partial write survives a loser. `test_review_data_maintenance.py` and
`test_review_cli.py` mirror the reference-data backup/restore and CLI tests
for the review store.

`test_review_api.py` and `test_review_session_api.py` cover session
resolution, login/logout, and the CSRF/origin double-submit contract through
HTTPX's ASGI transport. `test_review_case_creation_api.py` through
`test_review_case_decision_api.py` cover each mutating route's own
auth/CSRF/idempotency/not-found/conflict contract with a fake processing
service; `test_review_api_error_contracts.py` adds the cross-route
properties no single route test owns (a genuine idempotency conflict, the
generic validation envelope, an unavailable-store 503 on the real mounted
route, and `X-Request-ID` correlation on a review error).
`test_review_console_page.py` asserts the console's static markup exposes no
`innerHTML` usage and only reaches the routes above through `fetch`.

`test_review_case_creation_integration.py` retains the real processing
graph (like `test_processing_integration.py`) behind a real login and CSRF
flow. `test_review_authorization_integration.py` proves a rejected review
request never resolves processing or storage dependencies, mirroring
`test_upload_dependency_order.py`'s technique for the review route family.
`test_review_retry_recovery_integration.py` proves a case snapshot is
...
Review tests use fictional actor secrets and an in-memory or temporary-file
`ReviewActorDirectory`/`SQLiteReviewRepository`, never a real operator actor
file. Route-level tests inject a fake `ProcessingService`; the two
integration tests retain the real one behind fake OCR/extraction boundaries,
exactly like `test_processing_integration.py`. No test requires an HTTPS
origin or a real browser; the ASGI transport exercises the same routes a
browser would over plain HTTP within the test process.
...
| Review domain model, transition, or authorization rule | Pure-function shape/transition/authorization-matrix tests, Ruff lint, Ruff format check |
| Review persistence, migration, or maintenance | Temporary SQLite integration tests, concurrency race test where applicable, Ruff lint, Ruff format check |
| Review session, CSRF, or authentication | Authenticated ASGI success plus 401/403/503 paths that apply, constant-time comparison test where applicable |
| Review case API route | Authenticated ASGI success plus the route's own auth/CSRF/idempotency/conflict/not-found paths |
| Review console page | Structural markup test (no `innerHTML`) plus a manual or scripted in-browser check of the changed flow |
