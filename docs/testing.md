...
Veridoc uses pytest. The Phase 2 suite covers the FastAPI application, bounded
upload validation, temporary-file cleanup, typed OCR contracts, normalized page
images, structured invoice schemas, the typed extraction graph, the mocked
OpenAI adapter, and in-process OCR/extraction endpoints.
...
service, and API tests inject a fake `StructuredExtractor`. Tests must not
...
No static type checker or coverage threshold is configured in Phase 2. Do not
