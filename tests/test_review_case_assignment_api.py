"""In-process tests for PUT /review/cases/{case_id}/assignment."""
...
async def _assign(
    ...
    *,
    ...
    idempotency_key: str = "assign-key-1",
) -> httpx.Response:
...
@pytest.mark.anyio
async def test_reviewer_self_claims_an_unassigned_case(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        ...
        response = await _assign(client, headers, case_id, expected_version=1)

    assert response.status_code == 200
    body = response.json()
    assert body["status"] == "assigned"
    assert body["assignee_id"] == "reviewer-1"
...
@pytest.mark.anyio
async def test_reviewer_cannot_assign_a_case_to_someone_else(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        headers = await _authenticated_headers(client, secret=_REVIEWER_SECRET)
        case_id = await _create_case(client, headers)

        response = await _assign(
            client, headers, case_id, expected_version=1, actor_id="reviewer-2"
        )
...
@pytest.mark.anyio
async def test_admin_reassigns_an_already_assigned_case(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        reviewer_headers = await _authenticated_headers(client, secret=_REVIEWER_SECRET)
        case_id = await _create_case(client, reviewer_headers)
        first_claim = await _assign(
            client, reviewer_headers, case_id, expected_version=1
        )
        assert first_claim.status_code == 200

        admin_headers = await _authenticated_headers(client, secret=_ADMIN_SECRET)
        response = await _assign(
            client,
            admin_headers,
            case_id,
            expected_version=2,
            actor_id="reviewer-2",
            reason="Reviewer-1 is out of office.",
        )

    assert response.status_code == 200
    assert response.json()["assignee_id"] == "reviewer-2"


@pytest.mark.anyio
async def test_admin_reassignment_requires_a_reason(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        reviewer_headers = await _authenticated_headers(client, secret=_REVIEWER_SECRET)
        case_id = await _create_case(client, reviewer_headers)
        first_claim = await _assign(
        ...
        admin_headers = await _authenticated_headers(client, secret=_ADMIN_SECRET)
        response = await _assign(
            client, admin_headers, case_id, expected_version=2, actor_id="reviewer-2"
        )

    assert response.status_code == 422
    assert response.json()["detail"]["code"] == "review_reassignment_reason_required"


@pytest.mark.anyio
async def test_assign_rejects_a_stale_expected_version(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        headers = await _authenticated_headers(client, secret=_REVIEWER_SECRET)
        case_id = await _create_case(client, headers)

        response = await _assign(client, headers, case_id, expected_version=99)

    assert response.status_code == 409
    assert response.json()["detail"]["code"] == "review_case_version_conflict"


@pytest.mark.anyio
async def test_assign_returns_404_for_an_unknown_case(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        headers = await _authenticated_headers(client, secret=_REVIEWER_SECRET)

        response = await _assign(client, headers, "unknown-case", expected_version=1)
...
@pytest.mark.anyio
async def test_assign_requires_an_idempotency_key(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        headers = await _authenticated_headers(client, secret=_REVIEWER_SECRET)
        case_id = await _create_case(client, headers)
...
@pytest.mark.anyio
async def test_assign_requires_csrf_protection(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        headers = await _authenticated_headers(client, secret=_REVIEWER_SECRET)
        case_id = await _create_case(client, headers)
...
@pytest.mark.anyio
async def test_assign_is_idempotent_for_a_repeated_key(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        headers = await _authenticated_headers(client, secret=_REVIEWER_SECRET)
        case_id = await _create_case(client, headers)

        first = await _assign(client, headers, case_id, expected_version=1)
        second = await _assign(client, headers, case_id, expected_version=1)
