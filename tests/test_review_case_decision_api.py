"""In-process tests for POST /review/cases/{case_id}/decisions."""
...
async def _decide(
    ...
    *,
    ...
    decision: str = "accept",
    reason: str | None = "Amounts reconcile.",
    idempotency_key: str = "decide-key-1",
) -> httpx.Response:
...
@pytest.mark.anyio
async def test_assignee_decides_an_assigned_case(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        ...
        response = await _decide(client, headers, case_id, expected_version=2)

    assert response.status_code == 200
    body = response.json()
    assert body["status"] == "decided"
    assert body["version"] == 3
    assert body["events"][-1]["event_type"] == "case_decided"
    assert body["events"][-1]["decision"] == "accept"
    assert body["events"][-1]["reason"] == "Amounts reconcile."


@pytest.mark.anyio
async def test_admin_decides_a_case_assigned_to_someone_else(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        ...
        await _assign_to_self(client, reviewer_headers, case_id)
        ...
        response = await _decide(
            ...
            expected_version=2,
            decision="reject",
            reason="Vendor is on the deny list.",
        )

    assert response.status_code == 200
    assert response.json()["events"][-1]["decision"] == "reject"


@pytest.mark.anyio
async def test_decide_rejects_a_non_assignee_reviewer(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        ...
        response = await _decide(client, other_headers, case_id, expected_version=2)
...
@pytest.mark.anyio
async def test_decide_rejects_deciding_an_already_decided_case(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        headers = await _authenticated_headers(client, secret=_REVIEWER_SECRET)
        case_id = await _create_case(client, headers)
        await _assign_to_self(client, headers, case_id)
        first = await _decide(client, headers, case_id, expected_version=2)
...
@pytest.mark.anyio
async def test_decide_requires_a_reason(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        headers = await _authenticated_headers(client, secret=_REVIEWER_SECRET)
        case_id = await _create_case(client, headers)
        await _assign_to_self(client, headers, case_id)
...
@pytest.mark.anyio
async def test_decide_returns_404_for_an_unknown_case(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        headers = await _authenticated_headers(client, secret=_REVIEWER_SECRET)

        response = await _decide(client, headers, "unknown-case", expected_version=1)
...
@pytest.mark.anyio
async def test_decide_is_idempotent_for_a_repeated_key(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        headers = await _authenticated_headers(client, secret=_REVIEWER_SECRET)
        case_id = await _create_case(client, headers)
        await _assign_to_self(client, headers, case_id)
        ...
        first = await _decide(client, headers, case_id, expected_version=2)
        second = await _decide(client, headers, case_id, expected_version=2)
