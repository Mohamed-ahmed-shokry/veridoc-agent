"""In-process tests for GET /review/cases/{case_id}/evidence."""
...
@pytest.mark.anyio
async def test_read_evidence_returns_a_verifiable_bundle(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        ...
        response = await client.get(
            f"/review/cases/{case_id}/evidence", headers=headers
        )

    assert response.status_code == 200
    body = response.json()
    assert body["bundle_version"] == 1
    assert body["exported_by"] == "reviewer-1"
    assert body["case"]["case_id"] == case_id
    assert [event["event_type"] for event in body["case"]["events"]] == ["case_created"]

    verified = verify_evidence_bundle(response.text)
    assert verified.case.case_id == case_id
    assert verified.bundle_digest == body["bundle_digest"]


@pytest.mark.anyio
async def test_read_evidence_returns_404_for_an_unknown_case(
    tmp_path: Path,
) -> None:
    async with _client(tmp_path) as (client, _repository):
        headers = await _authenticated_headers(client)
        response = await client.get(
            "/review/cases/unknown-case/evidence", headers=headers
        )
...
@pytest.mark.anyio
async def test_read_evidence_requires_authentication(tmp_path: Path) -> None:
    async with _client(tmp_path) as (client, _repository):
        response = await client.get(
            "/review/cases/any-case/evidence", headers={"Origin": _ORIGIN}
        )
